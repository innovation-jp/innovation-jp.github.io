...
※Firestoreの使い方は、以下のガイドを参考にしてください。 +
...
これで完成です！
シンプルなものですが、リアルタイムで更新されるチャットアプリができました。

ただ、このままだとアプリを起動した時に、すでにデータベースに保存されているチャットの内容が、ランダムで並んでしまうので、時系列で並ぶように、投稿時間なども保存して並べる必要がありそうです。

ということで、またブログを書く機会があったらお目にかかりましょう！
