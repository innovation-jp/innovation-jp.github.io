...
構造体名称の先頭文字とメンバ変数が大文字となっているのは、別パッケージから参照可能なpublicj変数とするためです。
...
全件取得するFind関数です。返却値は複数件となるため、構造体配列へのポインタを渡します。
...
Model関数に更新対象テーブルとマッピングする構造体のポインタと、Update関数に更新する情報を設定します。 +
...
DBからの返却値は、ポインタを指定するところは変わりませんね。
...
===== 感想
まず、GOのお作法に若干戸惑いましたが、全般的には直感的に書けるので描きやすい言語だと思います。そして、標準ライブラリ（database/sql）との違いですが、DBからの返却値が簡単に操作できる点だと思いました。標準ライブラリでは、Scan関数にマッピングする項目を記載するのですが、gormではエンティティとマッピングする構造体のポインタを渡すとそこに値をセットしてくれます。これは楽！！。それとオープンソースなのでライブラリで実装されていることも確認可能。これはありだと思いました。
