...
何か四角があるな〜、と思うでしょうがこれが吹き出しです！ 吹き出しの三角は、弊社女性エンジニア（女神）のAMIさんの http://tech.innovation.co.jp/2017/03/03/C-S-S-triangle.html/[記事] を参考にしました。
...
```
*◆ 困った話 その2 ◆* +
作成したクラスをブランクページ作成時に自動生成された home.ts に インポートし、処理を実装したところ、特にコンパイルエラーも発生しなかったため、ブラウザで確認するとエラー画面が表示され `net.isIP is not a function` となっていました。
