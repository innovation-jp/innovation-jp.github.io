...
と遷移し、EC2ContainerService-xxxxxx-EcsInstanceLc-XXXXXXXX というような、
今回作成された launch configuration を選択し、 Copy launch configuration をクリックして
新しい launch configuration を作成します。

Launch configuration details > Edit details > Advanced Details > User data

と遷移し、起動時にDockerコンテナーのディスクを増やすオプションを追記するように指定します。
...
DesiredとMaxの台数を現在の2倍の数値としてスケールアウトさせ、
スケールアウト後もとの数値に戻すなどして、
